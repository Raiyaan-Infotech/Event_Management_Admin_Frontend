...
import {
    ...
    useCreateVideoTutorialDifficultyLevel,
    useUpdateVideoTutorialDifficultyLevel,
    useUpdateVideoTutorialDifficultyLevelStatus,
    useDeleteVideoTutorialDifficultyLevel,
    VideoTutorialDifficultyLevel
...
export function VideoTutorialDifficultyContent() {
    const { data: levels = [], isLoading } = useVideoTutorialDifficultyLevels();

    const createMutation = useCreateVideoTutorialDifficultyLevel();
    const updateMutation = useUpdateVideoTutorialDifficultyLevel();
    const toggleStatusMutation = useUpdateVideoTutorialDifficultyLevelStatus();
    const deleteMutation = useDeleteVideoTutorialDifficultyLevel();
    ...
    const [editLevel, setEditLevel] = useState<VideoTutorialDifficultyLevel | null>(null);
    const [deleteId, setDeleteId] = useState<number | null>(null);

    const [name, setName] = useState('');
    const [icon, setIcon] = useState('BarChart');
    ...
    const openCreateModal = () => {
        ...
        setDescription('');
        setColor('#22C55E');
        setSortOrder(levels.length + 1);
        setIsActive(true);
        setErrors({});
        setModalOpen(true);
    };

    const openEditModal = (lvl: VideoTutorialDifficultyLevel) => {
        setEditLevel(lvl);
        setName(lvl.name || '');
        setIcon(lvl.icon || 'BarChart');
        setDescription(lvl.description || '');
        setColor(lvl.color || '#22C55E');
        setSortOrder(lvl.sort_order ?? 1);
        setIsActive(Number(lvl.is_active) === 1 || lvl.is_active === true);
        setErrors({});
        setModalOpen(true);
    };
    ...
    const handleSave = () => {
        ...
        if (!name.trim()) newErr.name = 'Level name is required';
        ...
        if (editLevel) {
            updateMutation.mutate({ id: editLevel.id, data: payload }, {
    ...
    const handleToggleStatus = (lvl: VideoTutorialDifficultyLevel) => {
        const currentActive = Number(lvl.is_active) === 1 || lvl.is_active === true;
        toggleStatusMutation.mutate({ id: lvl.id, is_active: !currentActive });
    };
    ...
    const filteredLevels = useMemo(() => {
        return levels.filter((lvl) => {
            if (search.trim()) {
                ...
                const descMatch = lvl.description?.toLowerCase().includes(q);
                if (!nameMatch && !descMatch) return false;
            }
            if (selectedStatus !== 'all') {
                const isActiveBool = Number(lvl.is_active) === 1 || lvl.is_active === true;
                const targetActive = selectedStatus === '1' || selectedStatus === 'true';
                if (isActiveBool !== targetActive) return false;
            }
            return true;
        });
    }, [levels, search, selectedStatus]);

    const paginatedLevels = useMemo(() => {
        const start = (page - 1) * limit;
        return filteredLevels.slice(start, start + limit);
    }, [filteredLevels, page, limit]);
    ...
    return (
        <div className="space-y-6 max-w-7xl mx-auto pb-16 text-foreground">
            <PageLoader
                ...
                text={isLoading ? "Loading difficulty levels..." : "Saving Difficulty Levels..."}
            />
            {/* Header Bar */}
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-border/80 pb-4">
                <div>
                    <h1 className="text-2xl font-extrabold tracking-tight text-foreground">Difficulty Levels</h1>
                    <p className="text-xs text-muted-foreground mt-0.5">
                        Manage all video tutorial difficulty levels.
                    </p>
                </div>
                <Button
                    size="sm"
                ...
                <CardContent className="p-0">
                        ...
                        <table className="w-full text-left text-xs border-collapse">
                            <thead>
                                <tr className="border-b border-border/80 bg-muted/40 text-muted-foreground font-bold uppercase tracking-wider text-[11px]">
                                    ...
                                    <th className="py-3 px-4 min-w-[200px]">Level Name</th>
                            ...
                            <tbody className="divide-y divide-border/60">
                                {isLoading ? (
                                    ...
                                    <tr>
                                        <td colSpan={8} className="py-16 text-center text-muted-foreground">
                                            No difficulty levels found. Click <strong>+ Add Level</strong> to create one.
                                        </td>
                                    </tr>
                                ) : (
                                    paginatedLevels.map((lvl, idx) => {
                                        const globalIndex = (page - 1) * limit + idx + 1;
                                        const isActive = Number(lvl.is_active) === 1 || lvl.is_active === true;
                                        const lvlColor = lvl.color || '#22C55E';
                                        const formattedDate = lvl.created_at
                                            ? new Date(lvl.created_at).toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' })
                                            : '—';

                                        return (
                                            <tr key={lvl.id} className="hover:bg-muted/20 transition-colors">
                                                ...
                                                <td className="py-3.5 px-4 text-center">
                                                    <div
                                                        ...
                                                        style={{ backgroundColor: `${lvlColor}15`, color: lvlColor, border: `1px solid ${lvlColor}30` }}
                                                    >
                                                        <Icon icon={lvl.icon && lvl.icon.includes(':') ? lvl.icon : `lucide:${(lvl.icon || 'bar-chart').toLowerCase()}`} className="h-4 w-4" />
                                                    </div>
                                                </td>
                                                <td className="py-3.5 px-4">
                                                    <span
                                                        ...
                                                        style={{ backgroundColor: `${lvlColor}15`, color: lvlColor, border: `1px solid ${lvlColor}30` }}
                                                    >
                                                        <span className="h-2 w-2 rounded-full" style={{ backgroundColor: lvlColor }} />
                                                        {lvlColor}
                                                    </span>
                                                </td>
                                                <td className="py-3.5 px-4">
                                                    <div className="flex items-center gap-2">
                                                        <Switch checked={isActive} onCheckedChange={() => handleToggleStatus(lvl)} />
                                                ...
                                                <td className="py-3.5 px-4 text-right">
                                                    <div className="flex items-center justify-end gap-1.5">
                                                        ...
                                                        <RowTranslateButton
                                                            section="video-tutorial-difficulty-levels"
                    ...
                    {!isLoading && filteredLevels.length > 0 && (
                        <div className="p-4 border-t border-border/80">
                            {filteredLevels.length > limit ? (
                                <TablePagination
                                    pagination={{ page, totalPages, totalItems: filteredLevels.length, limit, hasNextPage: page < totalPages, hasPrevPage: page > 1 }}
                                    onPageChange={setPage}
                                    onLimitChange={(newLimit) => { setLimit(newLimit); setPage(1); }}
                                />
                            ) : (
                                <div className="flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-muted-foreground">
                                    <span>Showing 1 to {filteredLevels.length} of {filteredLevels.length} levels</span>
            ...
            {/* Add / Edit Difficulty Level Dialog Modal */}
            <Dialog open={modalOpen} onOpenChange={setModalOpen}>
                <DialogContent className="max-w-xl bg-card border-border text-foreground p-6 rounded-2xl shadow-xl">
                    <DialogHeader>
                        <DialogTitle className="text-lg font-extrabold text-foreground">
                            {editLevel ? 'Edit Difficulty Level' : 'Add Difficulty Level'}
                        </DialogTitle>
                        <DialogDescription className="text-xs text-muted-foreground">
                            Create a difficulty level to help users find tutorials that match their skill level.
                        </DialogDescription>
                    </DialogHeader>

                    <div className="space-y-5 pt-2">
                        {/* Section 1. Difficulty Level Information */}
                        <div className="space-y-4">
                            <h4 className="text-xs font-extrabold text-primary uppercase tracking-wider">1. Difficulty Level Information</h4>

                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    <Label className="text-xs font-bold text-foreground">Level Name <span className="text-rose-500">*</span></Label>
                                    <BuilderCountedInput
                                        value={name}
                                        onChange={(val) => {
                                            setName(val);
                                ...
                                <div className="space-y-1.5">
                                    <Label className="text-xs font-bold text-foreground">Level Icon</Label>
                                    <div className="flex items-center gap-2">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <BuilderCountedTextarea
                                    ...
                                    className="bg-background text-xs border-border"
                                />
                                <p className="text-[11px] text-muted-foreground">Describe what users can expect at this level.</p>
                            </div>
                        ...
                        <div className="space-y-4 pt-2 border-t border-border">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-center">
                                <div className="space-y-1.5">
                                    ...
                                    />
                                    <p className="text-[11px] text-muted-foreground">Set the order in which this level will appear.</p>
            ...
            <DeleteDialog
                ...
                description="Are you sure you want to delete this difficulty level?"
            />
        </div>
    );
}
