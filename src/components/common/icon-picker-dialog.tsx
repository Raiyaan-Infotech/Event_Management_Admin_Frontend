    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl flex flex-col overflow-y-hidden gap-3 p-4">
        ...
        <ScrollArea className="h-[380px] rounded-md border border-border">
          {loading ? (
            <div className="flex items-center justify-center h-40 text-sm text-muted-foreground">Loading icons…</div>
          ) : filtered.length === 0 ? (
            ...
            <div className="grid grid-cols-8 gap-0.5 p-2">
              {filtered.map(name => (
                <button key={name} type="button" title={name} onClick={() => handleSelect(name)}
                  ...
                  <span className="text-[9px] text-muted-foreground truncate w-full text-center group-hover:text-foreground leading-none">{name}</span>
                </button>
              ))}
            </div>
          )}
